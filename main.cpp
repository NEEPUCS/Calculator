...
Polyn CreatePolyn(Polyn head, int m)//建立一个头指针为head、项数为m的一元多项式（一个多项式最多不超过20项） 
{
	int i;
	Polyn p;
	p = head = (Polyn)malloc(sizeof(struct Polynomial));//动态分配内存 
	head->next = NULL;
	for (i = 0; i < m; i++)
	{
...
void main()
{
	...
	{
		printf("\n请选择操作：");
		scanf(" %c", &flag);//空格符号一定要注意,将缓冲区内的空格等符号影响消除
		switch (flag)
		{
		...
		case'k':
		{
			pa = (Polyn)malloc(sizeof(struct Polynomial));//建立新结点以接收数据
			pa->next = NULL;
			pb = (Polyn)malloc(sizeof(struct Polynomial));//建立新结点以接收数据
			pb->next = NULL;
			ReadFile(&pa, &pb);
			break;
		}
